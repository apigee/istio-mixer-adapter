...
to your organization and environment.`,
	...
	c.Flags().BoolVarP(&p.grpcHandler, "grpc", "g", false,
		"emit configuration for GRPC mixer handler")
...
func (p *provision) printApigeeHandler(cred *credential, printf shared.FormatFn, verifyErrors error) error {
	...
	if p.IsOPDK {
		spec.AnalyticsOptions = analyticsOptions{
			LegacyEndpoint: true,
		}
	}
	...
	if p.grpcHandler {
		...
		printf("# Istio handler configuration for Apigee adapter in Mixer")
	}
...
type grpcSpecification struct {
	Adapter    string        `yaml:"adapter"`
	Connection connection    `yaml:"connection"`
	Params     specification `yaml:"params"`
}
